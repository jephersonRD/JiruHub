...
const AppleIcon = () => (
  <svg fill="currentColor" strokeWidth="0" viewBox="0 0 1024 1024" className="w-4 h-4">
  ...
  return (
    ...
    >
      {platforms.map((platform) => (
        <motion.button
          key={platform.name}
          whileHover={platform.disabled ? undefined : { scale: 1.02 }}
          whileTap={platform.disabled ? undefined : { scale: 0.98 }}
          disabled={platform.disabled}
          onClick={platform.route ? () => navigate(platform.route) : undefined}
          className={`flex items-center gap-2 px-4 py-2 rounded-full border backdrop-blur-md transition-all ${
            platform.disabled
              ? 'bg-white/30 border-white/10 opacity-40 cursor-not-allowed'
              : 'bg-white/60 border-white/20 hover:bg-white/70 cursor-pointer'
          }`}
        >
          <platform.icon />
          <span
            className={`text-[14px] font-normal ${
              platform.disabled
                ? 'text-[rgba(30,50,90,0.5)]'
                : 'text-[rgba(30,50,90,0.9)]'
            }`}
          >
            {platform.name}
          </span>
