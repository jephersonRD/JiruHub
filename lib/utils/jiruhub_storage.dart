...
class JiruHubStorage {
  ...
  static ensureInitialized() async {
    _path = JiruHubDirectory.getDirectory;
    // 初始化设置
  ...
  static migrateV1ToV2() async {
    ...
    final miruList = await database.jiruHubDetails.where().findAll();
    database.writeTxn(() async {
      for (final detail in miruList) {
        ...
        if (tmdb != null) {
          ...
          await database.jiruHubDetails.put(detail);
        }
      }
    });
  }
  ...
  static Future<int> getDatabaseVersion() async {
    ...
    if (version == null) {
      final path = JiruHubDirectory.getDirectory;
  ...
  static _initSettings() async {
    ...
    final savedUrl = settings.get(SettingKey.jiruhubRepoUrl);
    if (savedUrl != null && savedUrl.toString().contains("jephersonrd.github.io")) {
      await settings.put(SettingKey.jiruhubRepoUrl, correctRepoUrl);
    }
    await _initSetting(SettingKey.jiruhubRepoUrl, correctRepoUrl);
...
class SettingKey {
  ...
  static const jiruhubRepoUrl = "JiruHubRepoUrl";
